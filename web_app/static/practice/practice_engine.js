/* ============================================================
   PRACTICE ENGINE – shared by every practice/exam page shell.
   Session state, data loading, question-type renderers, audio,
   scoring, and submit. Page shells (practice.js today, a sidebar
   shell later) only own the bottom-nav / navigation UI and call
   into this file, which is loaded first so its globals are ready.
   ============================================================ */
...
// ── Session loading ────────────────────────────────────────────
// Fetches the question set (multi-select / deep-linked progress group / full
// lesson) and populates session state. Returns false when the caller should
// stop silently (e.g. multi-mode redirected away because the queue was
// empty); throws on a real fetch failure so the shell's init() can alert.
...
// Pure queries over session state — reused by any shell's nav UI.
...
function checkAnswers() {
    ...
    if (entry) {
        entry.checked = true;
        entry.correctCount = groupCorrect;
        entry.correctTotal = group.questions.length;
    }
    updateNav();
    applyPracticeHanText(group);
}
